...
st.set_page_config(
    page_title="Numerical Root Finder",
    page_icon="🔢",
    layout="wide"
)
...
st.markdown("""
    <style>
    /* Global theme variables - teal/emerald palette */
    :root{
        --bg-1: #ecfeff; /* light background */
        --bg-2: #f0fdfa;
    ...
    /* High-specificity overrides to ensure theme consistency */
    body, .main, .stApp, .block-container { background: var(--bg-1) !important; color: var(--text-dark) !important; }
    [data-testid="stSidebar"] { background: linear-gradient(180deg,var(--bg-2),#ecfeff) !important; }
    .force-header, .app-header, .intro-card, [data-testid="stMetricContainer"], .dataframe thead tr th { border-radius: 10px !important; }

    /* Buttons */
    .stButton > button[kind="primary"] { background: linear-gradient(135deg,var(--primary-1),var(--primary-2)) !important; box-shadow: 0 8px 24px rgba(6,182,212,0.12) !important; color: var(--text-light) !important; }
    .stButton > button { border-radius: 10px !important; }

    /* Inputs and controls */
    .stTextInput input, .stNumberInput input, .stTextArea textarea { border: 2px solid var(--glass) !important; box-shadow: 0 6px 20px rgba(14,165,164,0.04) !important; }
    .stTextInput input:focus, .stNumberInput input:focus, .stTextArea textarea:focus { border-color: var(--primary-1) !important; box-shadow: 0 8px 30px rgba(14,165,164,0.12) !important; }

    /* Accent text and metric colors */
    [data-testid="stMetricValue"] { color: var(--text-light) !important; font-weight: 700 !important; font-size: 1.5rem !important; }
    [data-testid="stMetricContainer"] { background: linear-gradient(135deg,var(--primary-1),var(--accent)) !important; padding: 20px !important; border-radius: 10px !important; }
    [data-testid="stMetricLabel"] { color: var(--text-light) !important; font-weight: 600 !important; }

    /* Tables */
    .dataframe thead tr th { background: linear-gradient(135deg,var(--primary-1),var(--primary-2)) !important; color: var(--text-light) !important; }

    /* Intro modal tweaks */
    .intro-card { background: linear-gradient(135deg,var(--primary-1),var(--accent)) !important; color: var(--text-light) !important; }

    /* Links and pills */
    .nav-pill, .intro-link { background: rgba(255,255,255,0.06) !important; border: 1px solid rgba(255,255,255,0.06) !important; }

    /* Fallback for any remaining purple/gold accents - neutralize */
    [style*="#667eea"], [style*="#764ba2"], [style*="#ffd700"], [style*="#ffed4e"] { background: none !important; border-color: transparent !important; color: inherit !important; }
    ...
    * {
    ...
    .stButton > button[kind="primary"]:hover {
        ...
        box-shadow: 0 6px 25px rgba(14, 165, 164, 0.45);
    }
    ...
    .dataframe tbody tr:hover {
        background-color: rgba(14, 165, 164, 0.03) !important;
        transform: scale(1.01);
    }
    
    .dataframe tbody tr td {
        padding: 12px !important;
        border-bottom: 1px solid rgba(14, 165, 164, 0.06);
    }
    ...
    [data-testid="stSidebar"] {
        ...
        border-right: 2px solid rgba(14, 165, 164, 0.08);
    }
    ...
    .streamlit-expanderHeader {
        ...
        font-weight: 600;
        padding: 14px 16px;
        background: linear-gradient(135deg, rgba(14,165,164,0.08) 0%, rgba(16,185,129,0.08) 100%);
        border: 1px solid rgba(14, 165, 164, 0.12);
        transition: all 0.3s ease;
        margin-bottom: 8px;
    }
    ...
    .streamlit-expanderHeader:hover {
        background: linear-gradient(135deg, rgba(14,165,164,0.12) 0%, rgba(16,185,129,0.12) 100%);
    ...
    hr {
        border: none;
        height: 2px;
        background: linear-gradient(90deg, transparent 0%, rgba(14,165,164,0.22) 50%, transparent 100%);
        margin: 2rem 0;
    }
    ...
    if calculate:
        ...
        # SINGLE METHOD MODE
            ...
            if result['success']:
                ...
                # Metrics with explicit labels
                st.markdown("### 📊 Results Summary")
                col1, col2, col3, col4 = st.columns(4)
                root_val = result['root']
                
                with col1:
                    st.markdown("<div style='text-align: center; background: linear-gradient(135deg,#0ea5a4,#10b981); color: white; padding: 10px; border-radius: 8px; font-weight: bold;'>ROOT</div>", unsafe_allow_html=True)
                    col1.metric("", f"{root_val:.8f}" if number_format == "Decimal" else f"{root_val:.6e}")
                
                if method == "Fixed Point Method":
                    with col2:
                        st.markdown("<div style='text-align: center; background: linear-gradient(135deg,#0ea5a4,#10b981); color: white; padding: 10px; border-radius: 8px; font-weight: bold;'>F(ROOT)</div>", unsafe_allow_html=True)
                        col2.metric("", f"{f(root_val):.2e}")
                    # Add verification
                    st.markdown("---")
                ...
                with col3:
                    st.markdown("<div style='text-align: center; background: linear-gradient(135deg,#0ea5a4,#10b981); color: white; padding: 10px; border-radius: 8px; font-weight: bold;'>ITERATIONS</div>", unsafe_allow_html=True)
                    col3.metric("", len(result['iterations']))
                
                with col4:
                    st.markdown("<div style='text-align: center; background: linear-gradient(135deg,#0ea5a4,#10b981); color: white; padding: 10px; border-radius: 8px; font-weight: bold;'>TOLERANCE</div>", unsafe_allow_html=True)
                    col4.metric("", f"{tolerance:.2e}")
        ...
        with col1:
            # Dynamic About section based on selected method
            if method == "Bisection Method":
                st.markdown("### 🎓 About Bisection Method")
                st.markdown("""
                Root-finding by repeatedly halving an interval where sign changes occur.
                
                **Key Steps:** Find [a,b] with sign change → Bisect → Check midpoint → Update interval
                
                **Pros:** Guaranteed convergence, simple | **Cons:** Slow, needs sign change
                """)
            elif method == "False Position Method":
                st.markdown("### 🎓 About False Position Method")
                st.markdown("""
                Improves bisection by using a weighted midpoint based on function values.
                
                **Key Steps:** Use linear interpolation through endpoints → Evaluate → Update interval
                
                **Pros:** Faster than bisection | **Cons:** May be slower in some cases
                """)
            elif method == "Newton-Raphson Method":
                st.markdown("### 🎓 About Newton-Raphson Method")
                st.markdown("""
                Uses function derivative for rapid convergence with quadratic rate.
                
                **Key Steps:** Start with x₀ → Apply formula: xₙ₊₁ = xₙ - f(xₙ)/f'(xₙ) → Iterate
                
                **Pros:** Fast convergence, quadratic rate | **Cons:** Needs derivative, may diverge
                """)
            elif method == "Secant Method":
                st.markdown("### 🎓 About Secant Method")
                st.markdown("""
                Similar to Newton-Raphson but approximates derivative using two points.
                
                **Key Steps:** Use two initial points → Approximate slope → Apply iterative formula
                
                **Pros:** No derivative needed | **Cons:** Slower than Newton, needs two points
                """)
            elif method == "Fixed Point Method":
                st.markdown("### 🎓 About Fixed Point Method")
                st.markdown("""
                Rearranges equation to form x = g(x) and iterates to find fixed point.
                
                **Key Steps:** Rearrange f(x)=0 → Define g(x) → Iterate: xₙ₊₁ = g(xₙ)
                
                **Pros:** Simple, flexible | **Cons:** May diverge if |g'(x)| > 1
                """)
        ...
        with col2:
            ...
            if method == "Bisection Method":
                ...
                st.caption("Weighted midpoint formula")
            elif method == "Newton-Raphson Method":
                st.latex(r"x_{n+1} = x_n - \frac{f(x_n)}{f'(x_n)}")
                st.caption("Newton-Raphson iteration")
            elif method == "Secant Method":
                st.latex(r"x_{n+1} = x_n - \frac{f(x_n)(x_n - x_{n-1})}{f(x_n) - f(x_{n-1})}")
                st.caption("Secant method formula")
            elif method == "Fixed Point Method":
                st.latex(r"x_{n+1} = g(x_n)")
                st.caption("Fixed point iteration")
# ============================================================================
...
```
    ...
    ```
        ...
        st.markdown("""
        Polynomial interpolation that passes through all given data points.
        
        **Formula:** P(x) = Σ y_i · L_i(x) where L_i(x) = ∏ (x - x_j)/(x_i - x_j)
        
        **Pros:** Simple, any number of points | **Cons:** High degree for many points
        """)
...
# ============================================================================
        ...
        if calculate_dd:
            ...
            st.markdown("""
            Newton form interpolation using divided differences. Easier to add new points than Lagrange.
            
            **Form:** P(x) = a₀ + a₁(x-x₀) + a₂(x-x₀)(x-x₁) + ...
            
            **Pros:** Incremental, efficient | **Cons:** More computation than Lagrange initially
            """)
...
# ============================================================================
    ...
    if calculate_jacobi:
        ...
        col1.metric("Converged", "✅ Yes" if result_jacobi['converged'] else "❌ No", label_visibility="visible")
        col2.metric("Iterations", len(result_jacobi['iterations']) - 1, label_visibility="visible")
        col3.metric("Final Error", f"{result_jacobi['final_error']:.2e}", label_visibility="visible")
        
        # Calculate final residual
        final_residual = np.linalg.norm(np.dot(A, result_jacobi['solution']) - b, ord=np.inf)
        col4.metric("Residual ||Ax-b||", f"{final_residual:.2e}", label_visibility="visible")
        ...
        st.markdown("""
        Iterative method for solving Ax = b by updating all variables simultaneously using old values.
        
        **Update:** x_i^(k+1) = (b_i - Σ a_ij·x_j^(k)) / a_ii
        
        **Pros:** Simple, parallel-friendly | **Cons:** Slow convergence
        """)
...
# Add this after the Jacobi section (or as a separate option)
    ...
    if calculate_gs:
        ...
        col1.metric("Converged", "✅ Yes" if result_gs['converged'] else "❌ No", label_visibility="visible")
        col2.metric("Iterations", len(result_gs['iterations']) - 1, label_visibility="visible")
        col3.metric("Final Error", f"{result_gs['final_error']:.2e}", label_visibility="visible")
        
        # Calculate final residual
        final_residual_gs = np.linalg.norm(np.dot(A_gs, result_gs['solution']) - b_gs, ord=np.inf)
        col4.metric("Residual ||Ax-b||", f"{final_residual_gs:.2e}", label_visibility="visible")
        ...
        st.markdown("""
        Improved version of Jacobi that uses updated values immediately in same iteration.
        
        **Update:** x_i^(k+1) = (b_i - Σ_j<i a_ij·x_j^(k+1) - Σ_j>i a_ij·x_j^(k)) / a_ii
        
        **Pros:** Faster than Jacobi | **Cons:** Sequential, cannot parallelize
        """)
...
# ============================================================================
    ...
    if calculate_diff:
        ...
        st.markdown("""
Approximate derivatives from discrete data points or complex functions.

**Methods:** Newton forward/backward differences using Δ or ∇ operators.

**Pros:** No analytical function needed | **Cons:** Noise-sensitive, requires uniform spacing
        """)
...
# ============================================================================
    ...
    st.markdown("""
Approximate definite integrals using weighted sums of function values at data points.

**Methods:** Trapezoidal, Simpson's 1/3, Simpson's 3/8 rules with varying accuracy.

**Pros:** Simple, works with tabular data | **Cons:** Requires equally spaced points, error increases with function complexity
    """)
...
st.caption("Numerical Computing Project | Numerical Root Finder")
